...
Accurate @ IoU≥0.5:          67%
Issues Identified
Low Calibration AUC (0.500): Confidence scores were uncorrelated with actual fix accuracy
Limited Search Window: Only ±10px with 2px steps (121 candidates) missed optimal alignments
Aggressive Gaussian Filtering: σ=1.0 on crisp binary boundaries destroyed edge definition
Boundary Noise: Raster analysis revealed 5,584 connected components (median 24px each)
...
Problem: σ=1.0 blurs crisp binary edges over ~3px radius Solution: Reduce to σ=0.3 for lighter smoothing
...
Problem: Limited translation search (±10px, 2px steps) missed optimal alignments Solution: Expand to ±12px with 1px steps
...
Problem: Hardcoded confidence buckets uncorrelated with accuracy (AUC = 0.5) Solution: Continuous confidence scoring based on edge_score × area_ratio
...
Village has different characteristics
6 ground truth examples too small to measure change
Improvements help other villages more
...
Open https://bhume.org/score
Upload m/data/predictions.geojson
Verify metrics
Upload v/data/predictions.geojson
Compare scores
...
13. LESSONS LEARNED
Directory structure matters: Python module imports depend on correct path
Raster data characteristics: Binary vs. smooth requires different filtering
Confidence scoring: Continuous ranking better than bucketing for AUC
Search thoroughness: 5× more candidates (625 vs. 121) still runs fast
Ground truth validation: 6 examples too small; trust logic + hidden set
14. RECOMMENDATIONS
✅ Submit: Use solution.py (ULTRAFAST) as primary submission
✅ Monitor: Track hidden set scores for validation
✅ Iterate: If hidden set < expected, try solution_medium.py
✅ Document: Log all submissions with dates and metrics
✅ Backup: Keep original predictions for comparison
...
Document Prepared By: Saikiran Panduri Date: June 14, 2026 Version: 1.0 Final Classification: Technical Implementation Report
